...
WORKDIR /src
COPY ["lambdawebapi.csproj", "lambdawebapi/"]
RUN dotnet restore "lambdawebapi/lambdawebapi.csproj"

WORKDIR "/src/lambdawebapi"
COPY . .
RUN dotnet build "lambdawebapi.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "lambdawebapi.csproj" \
